...
def dump_results(args, scanrefer, data, config):
    dump_dir = os.path.join(CONF.PATH.OUTPUT, args.folder, "vis_nogt")
    ...
    nyu40_label = torch.argmax(data["ref_obj_cls_scores"], dim=1).detach().cpu().numpy() + 1
...
def visualize(args):
    ...
    for data in tqdm(dataloader):
        ...
        # feed
        with torch.no_grad():
            data = model.votenet_extractor(data)
            centers = data["aggregated_vote_xyz"][0]
            size_classes = torch.argmax(data['size_scores'][0], dim=1, keepdim=True)
            sizes = torch.gather(data["size_residuals"][0], dim=1,
                                 index=size_classes.unsqueeze(2).expand(-1, 1, 3)).squeeze(1)
            objectness = torch.softmax(data["objectness_scores"][0], dim=-1)[:, 1]

            sizes = torch.stack(
                [centers.new_tensor(DC.param2obb(center.numpy(), 0, 0, cls.numpy(), size.numpy()))[3:6] for
            ...
            for objectness, center, size in zip(objectness, centers, sizes):
                ...
                data["object_id"] = center.new_tensor([object_counter], dtype=torch.int32)
                data["ann_id"] = center.new_tensor([0], dtype=torch.int32)
                data = model(data)
